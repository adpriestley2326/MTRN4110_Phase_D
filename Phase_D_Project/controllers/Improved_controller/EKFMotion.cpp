// EKFMotion.cpp
// Terry's Phase A EKF Controller Code

#include <EKFMotion.hpp>
#include <string>
#include <iostream>
#include <sstream>
#include <iomanip>
...
HatTrickController::HatTrickController(FloodFillMap& map) {
  this->map = map;
  this->map.doFloodFill();
  Path best_path = map.fewestTurnsPath(this->map.curr_cell, this->map.curr_dir);
  this->motionPlan = generateMotionPlan(best_path, this->map.curr_cell, this->map.curr_dir);
  this->instructionQ = generateInstructionQueue(this->motionPlan);
  this->robot = new Robot();
  this->initialiseSensors();
...
void HatTrickController::doUpdate() {
  ...
  if (idleCount >= 1) {
    ...
    // Check walls
    bool left_wall_present = dsLeft->getValue() < WALL_THRESHOLD;
    bool front_wall_present = dsFront->getValue() < WALL_THRESHOLD;
    bool right_wall_present = dsRight->getValue() < WALL_THRESHOLD;
    ...
    /*std::cout << MESSAGE_PREFIX << "Step: " << std::setw(3) << std::setfill('0') << motionPlanStep - 3
              ...
              << ", Left Wall: " << (left_wall_present ? "Y":"N")
              << ", Front Wall: " <<  (front_wall_present ? "Y":"N")
              << ", Right Wall: " << (right_wall_present ? "Y":"N")
              << "\n";*/
    ...
    */
    ...
    // Add any detected walls to maze
    bool should_update_map = false;
    if (left_wall_present) {
      map.addWall(map.curr_cell, turn(map.curr_dir, true));
      should_update_map = true;
    }
    
    if (front_wall_present) {
      map.addWall(map.curr_cell, map.curr_dir);
      should_update_map = true;
    }
    
    if (right_wall_present) {
      map.addWall(map.curr_cell, turn(map.curr_dir, false));
      should_update_map = true;
    }
    
    // Generate new path from updated maze, starting with the current robot pose
    if (should_update_map) {
      map.updateFloodFill(map.curr_cell);
      Path newPath = map.fewestTurnsPath(map.curr_cell, map.curr_dir);
      if (!newPath.empty()) {
        map.highlightPath(newPath);
        map.display();
        map.doFloodFill();
        std::string newMotionPlan = generateMotionPlan(newPath, map.curr_cell, map.curr_dir);
        cout << newMotionPlan << "\n";
        instructionQ = generateInstructionQueue(newMotionPlan);
        cout << instructionQ.front() << "\n\n";
      }
    }
    
    if (/*motionPlanStep < motionPlan.length()*/ !instructionQ.empty()) {
      // Advance to next step if one exists, else complete
      //action = actionList.find(motionPlan[motionPlanStep]);
      action = actionList.find(instructionQ.front());
      instructionQ.pop();
      switch(action) {
        case(0):
          move_forward(target);
          map.curr_cell = map.getNeighbourCell(map.curr_cell, map.curr_dir);
          break;
        case(1):
          target(2) += M_PI/2;
          map.curr_dir = turn(map.curr_dir, true);
          break;
        case(2):
          target(2) -= M_PI/2;
          map.curr_dir = turn(map.curr_dir, false);
          break;
      } 
      positioned = false;
      rotated = false;
