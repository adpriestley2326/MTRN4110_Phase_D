...
int main(int argc, char **argv) {
  ...
  Path fewest_turns_path = map.fewestTurnsPath(map.curr_cell, map.curr_dir); // CRASHES HERE
  map.highlightPath(fewest_turns_path);
  map.display();
  myPrint("Shortest path with least turns found!");
  
  // Generate motion plan
  string motion_plan = generateMotionPlan(fewest_turns_path, map.curr_cell, map.curr_dir);
  int motion_plan_length = motion_plan.length() - 3;
  myPrint("Path Plan (" + to_string(motion_plan_length) + " steps): " + motion_plan);
  
  
  // Write motion plan to text file
  ...
  HatTrickController *robot = new HatTrickController(map);
